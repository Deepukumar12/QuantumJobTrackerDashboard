import type { 
  Notification as DbNotification, InsertNotification, UserProfile, Job, Project, 
...
export interface Notification extends Omit<DbNotification, 'category' | 'actionUrl' | 'actionLabel' | 'readAt'> {
  category?: string;
...
export class SmartNotificationService {
  ...
  private async createBatchedNotification(notifications: Notification[]): Promise<void> {
    ...
    for (const notification of notifications) {
      notification.data = { ...(notification.data as any), batched: true };
    }
  }
